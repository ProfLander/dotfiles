...
- [-] Look into niri again
  ...
  - [ ] Window rules for moving 'service windows' to dedicated workspaces
    - [ ] Emacs
    - [ ] OBS
      - Can use key shortcuts + tray icon / persistent notification as primary interface
      - Implement a minibuffer setup for switching scenes
      - How to manage the desktop portal screen capture popup?
        - Ideally should be configured once to capture DP-1
  - [ ] Experiment with niri's built-in screen capture machinery
